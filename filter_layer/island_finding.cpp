...
void findBiggestIsalnd(int commonColor){
    ...
    for(int j = 0; j < col; j++){
        for(int i = 0; i < row; i++){
            ...
            if(visited[i][j] && data[i][j] > data[i][j] -3 && data[i][j] < data[i][j] + 3){//if it has not been visited and the color is the common color
                Island currIsland;
...
*/
void expandIsland(int x, int y, Island currIsland){
...
//have a santatizing method

void comparingIslands(vector< vector<Island> > frames){
   //Cases that come to mind
   //When user leaves screen
    
   //could compare each island against every island
    
   //things to look for, in this island do we have an island that like with excatly like it (with resonable deviation)?
    for(int i = 0; i < frames.size(); i++){
     //decide on the change in measure    
    }
